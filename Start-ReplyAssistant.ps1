param(
    [ValidateSet('status','start','stop','doctor','restart-check','backup','freeze','verify-release','restore-check','preview','reply','handoffs')]
...
if ($Action -eq 'reply' -and -not $ConfirmSend) { throw '真实发送需要当前用户明确要求并指定 -ConfirmSend。' }
