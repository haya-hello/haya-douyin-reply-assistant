...
- 私信批次已接入：现取历史快照，按会话合并尚未回答的新入站，生成后再刷新；手动回过或对话变化则跳过，发送必须获得接口成功与本人回显。没有安全候选时不发送。不启用旧DMShoot无限自动回复。
...
6. 私信同会话最多自动2次，第二次须对方新入站；已确认的旧工具测试发送也计入次数。不确定会话交本人，消息结构或时间不可靠时不发送。
...
.\Start-ReplyAssistant.ps1 -Action doctor-dm -Online
.\Start-ReplyAssistant.ps1 -Action dm-preview
```
...
只有用户本次明确要求“帮我回私信”，才执行：

```powershell
.\Start-ReplyAssistant.ps1 -Action dm-reply -ConfirmSend
```

私信入口使用DMShoot独立账号凭据和新鲜快照，不要求Chrome评论脚本在线；会临时打开独立Edge读取历史，结束即关闭。当前快照非全量历史，不可发送来自过期缓存或推测会话的消息。
...
- `data/releases/v1.1.0/manifest.json`：当前源码哈希、上游提交、运行版本；v1.0.0原包保留不变。
...
正常使用无需改代码。升级走新版本、回归与冻结，不覆盖旧包。V1.1私信批次已实现；真实快照与预览可验收，模拟批次已覆盖安全规则，不把没有实际发生的批量外发说成已经验收。
